...
-- ============================================================
--  إعلان ما قبل التشغيل (Pre-roll): يظهر عند فتح مباراة لأول مرة، قبل تحميل
--  البث الفعلي، لمدة محدودة (10-20 ثانية). يُدار بالكامل من لوحة الأدمن —
--  لو ad_enabled = false أو ما فيه رابط صورة/فيديو، ما يظهر شي والمشغل
--  يكمل تحميل البث فورًا زي ما كان قبل هالتحديث.
-- ============================================================
alter table public.app_settings add column if not exists ad_enabled boolean default false;
alter table public.app_settings add column if not exists ad_image_url text default '';
alter table public.app_settings add column if not exists ad_video_url text default '';
alter table public.app_settings add column if not exists ad_click_url text default '';
alter table public.app_settings add column if not exists ad_duration_seconds int default 15;
alter table public.app_settings add column if not exists ad_skip_after_seconds int default 5;
alter table public.app_settings add column if not exists ad_network_script text default '';

-- ============================================================
--  بانر إعلاني ثابت أسفل الشاشة (Sticky Bottom Banner): يظهر فوق صفحة
--  اختيار الأندية (index.html) وصفحة المباراة (player.html) — خارج مشغل
--  الفيديو نفسه. فيه زر إغلاق يخفيه لبقية الجلسة فقط. يُدار بالكامل من
--  لوحة الأدمن — لو banner_ad_enabled = false أو ما فيه صورة/كود، ما يظهر شي.
-- ============================================================
alter table public.app_settings add column if not exists banner_ad_enabled boolean default false;
alter table public.app_settings add column if not exists banner_ad_image_url text default '';
alter table public.app_settings add column if not exists banner_ad_click_url text default '';
alter table public.app_settings add column if not exists banner_ad_network_script text default '';

-- ============================================================
--  إعلان جانبي دوري داخل المشغل (Side Ad، أثناء التشغيل): بعكس إعلان
--  ما قبل التشغيل (ad_*)، هذا يظهر بشكل دوري أثناء المشاهدة نفسها كل
--  side_ad_interval_seconds، ويستمر side_ad_duration_seconds ثم يختفي —
--  بدون ما يوقف أو يعيد تحميل البث إطلاقًا. المشغل يتصغّر بانتقال سلس
--  (CSS transition) ليفسح مساحة لصندوق الإعلان بجانبه، وبعدها يرجع لحجمه
--  الكامل بنفس السلاسة. التوقيت عالمي موحّد لكل المشاهدين معًا (يبدأ من created_at
--  بجدول match_sources — لحظة إضافة رابط السيرفر بلوحة الأدمن نفسها)،
--  بغض النظر عن مدة متابعة المستخدم الفعلية. راجع js/player/12-side-ad.js.
-- ============================================================
alter table public.app_settings add column if not exists side_ad_enabled boolean default false;
alter table public.app_settings add column if not exists side_ad_interval_seconds int default 1200;
alter table public.app_settings add column if not exists side_ad_duration_seconds int default 20;
alter table public.app_settings add column if not exists side_ad_image_url text default '';
alter table public.app_settings add column if not exists side_ad_click_url text default '';
alter table public.app_settings add column if not exists side_ad_network_script text default '';
