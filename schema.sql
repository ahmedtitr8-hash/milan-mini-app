...
-- ============================================================
--  Referer مخصص لكل سيرفر (بعض السيرفرات تطلبه عشان تسمح بالبث) +
--  اختيار صريح لأي بروكسي يمر عليه السيرفر (بدل التخمين التلقائي من شكل الرابط)
--  فاضي/auto = زي ما هو الوضع الحالي بالضبط، ما يتغير شي على أي سيرفر موجود
-- ============================================================
alter table public.match_sources add column if not exists referer text;
alter table public.match_sources add column if not exists proxy_mode text not null default 'auto';
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'match_sources_proxy_mode_check') then
    alter table public.match_sources add constraint match_sources_proxy_mode_check check (proxy_mode in ('auto','cloudflare','supabase'));
  end if;
end $$;
