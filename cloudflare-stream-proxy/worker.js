    ...
    if (isManifest) {
      ...
      const rewritten = text
        ...
        .map((line) => {
          ...
          const refSuffix = refererParam ? `&referer=${encodeURIComponent(refererParam)}` : "";
          if (trimmed.startsWith("#")) {
            return line.replace(/URI="([^"]+)"/i, (_m, uri) => {
              try {
                const abs = new URL(uri, targetUrl).toString();
                return `URI="${proxyBase}?url=${encodeURIComponent(abs)}&ua=${uaKey}${refSuffix}"`;
              } catch {
                return _m;
              }
            });
          ...
          try {
            ...
            return `${proxyBase}?url=${encodeURIComponent(abs)}&ua=${uaKey}${refSuffix}`;
          } catch {
            return line;
          }
        })
