...
export default {
  async fetch(req) {
    ...
    if (req.method !== "GET" && req.method !== "HEAD") {
      ...
      // تشخيص مؤقت: لو سيرفر الترخيص رفض الطلب (أي كود غير 2xx)، ما فيه فايدة نمرر الجسم الخام
      // زي ما هو أصلًا (شاكا رح تفشل بكل الأحوال) — فنلفّه بـJSON مقروء يوضح كود الحالة ونص الرد
      // الحقيقي (أول 2000 حرف)، عشان تقدر تشوفه مباشرة بأي أداة فحص شبكة بلا Developer Tools.
      // لو الحالة 2xx (رخصة حقيقية ناجحة)، يمر خام زي ما هو دايمًا، ما نأثر على التشغيل الفعلي إطلاقًا.
      if (!licenseResp.ok) {
        const bodyText = await licenseResp.text().catch(() => '');
        return new Response(JSON.stringify({
          debug_license_upstream_status: licenseResp.status,
          debug_license_upstream_statusText: licenseResp.statusText,
          debug_license_upstream_body_preview: bodyText.slice(0, 2000),
          debug_target_url: targetUrl.toString(),
        }, null, 2), {
          status: 200,
