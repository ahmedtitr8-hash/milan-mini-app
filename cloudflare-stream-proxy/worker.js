...
export default {
  async fetch(req) {
    ...
    if (isMpd) {
      ...
      const proxify = (raw) => {
        try {
          // نحافظ على متغيرات SegmentTemplate ($Number$, $Time$, $RepresentationID$, $Bandwidth$)
          // بدون تشفير — encodeURIComponent يحوّل "$" إلى "%24" فيفسد المتغيرات، فيفشل dash.js/Shaka
          // بالتعرف عليها واستبدالها برقم المقطع الفعلي (هذا سبب فشل روابط SegmentTemplate تحديدًا
          // بينما SegmentList/SegmentURL بروابط صريحة تشتغل عادي لأنها ما فيها متغيرات أصلًا)
