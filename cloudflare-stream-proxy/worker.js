...
const UA_PRESETS = {
  ...
  // متصفح ويب عادي — مو موجود قبل (كل الخيارات كانت تطبيقات IPTV/مشغلات). بعض
  // خدمات الاستضافة العامة (بوتات تحويل فيديو تيليجرام لرابط مثلًا) تحجب توقيعات
  // تطبيقات التحميل (okhttp) تحديدًا، وتقبل متصفح حقيقي عادي بدون مشاكل.
...
export default {
  async fetch(req) {
    ...
    // نمرّر طلب الجزء (Range) من المتصفح للمصدر الأصلي — كانت ناقصة بهذي النسخة (موجودة
    // بنسخة Supabase) وبدونها بعض مصادر Xtream/IPTV والفيديوهات الكبيرة ترفض الطلب أو تتقطع
