...
export default {
  async fetch(req) {
    ...
    // بعض السيرفرات (خصوصًا منصات محمية زي Intigral/STC) ترفض أي طلب بلا Referer (حماية hotlink) —
    // النسخة القديمة كانت ترسل بلا Referer إطلاقًا. الافتراضي الآن نفس أصل الرابط الهدف نفسه،
    // وتقدر تفرض Referer معين صراحة بـ ?ref=https://example.com لو المصدر يشترط نطاق محدد بعينه
    const refParam = reqUrl.searchParams.get("ref");
    const referer = refParam || `${targetUrl.protocol}//${targetUrl.host}/`;
      ...
      const rewritten = text
        ...
        .map((line) => {
          ...
          if (trimmed.startsWith("#")) {
            return line.replace(/URI="([^"]+)"/i, (_m, uri) => {
              try {
                ...
                return `URI="${proxyBase}?url=${encodeURIComponent(abs)}&ua=${uaKey}${refQS}"`;
              } catch {
                return _m;
              }
            });
          ...
          try {
            ...
            return `${proxyBase}?url=${encodeURIComponent(abs)}&ua=${uaKey}${refQS}`;
          } catch {
            return line;
          }
        })
    ...
    if (isDashManifest) {
      ...
      const wrapXml = (raw) => {
        try {
          ...
          const proxied = `${proxyBase}?url=${encoded}&ua=${uaKey}${refQS}`;
