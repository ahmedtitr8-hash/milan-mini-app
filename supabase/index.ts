...
Deno.serve(async (req: Request) => {
  ...
  // نقرأ أول جزء فقط من الرد (لا الرد كامل) لنتحقق فعليًا هل هو نص مانيفست HLS أو DASH أم بيانات
  // ثنائية — هذا أدق من الاعتماد على الامتداد/الترويسة فقط، وما يزال يحافظ على التدفق الحقيقي
  // للمقاطع الكبيرة
  ...
  // DASH: نفس منطق نسخة Cloudflare حرفيًا — يعتمد على أول بايتات الرد (XML) لا اسم الملف إطلاقًا
  // (كانت هذي النسخة ناقصة دعم mpd بالكامل — تمرّر المانيفست خام بدون إعادة كتابة روابط المقاطع
  // الداخلية جواه، فحتى لو نجح جلب المانيفست، التشغيل ينكسر بعدها على أول مقطع)
