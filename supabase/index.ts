  ...
  try {
    ...
    const upstreamResponse = await fetch(targetUrl, {
      ...
      headers: {
        ...
        ...(refererParam ? { 'Referer': refererParam } : {}),
      },
      redirect: 'follow',
    })
      ...
      const rewritten = manifestText.split('\n').map(line => {
        ...
        if (trimmed.startsWith('#')) {
          ...
          if (uriMatch) {
            ...
            const proxied = `${reqUrl.origin}${reqUrl.pathname}?url=${encodeURIComponent(abs)}${uaParam ? `&ua=${uaParam}` : ''}${refererParam ? `&referer=${encodeURIComponent(refererParam)}` : ''}`
        ...
        return `${reqUrl.origin}${reqUrl.pathname}?url=${encodeURIComponent(abs)}${uaParam ? `&ua=${uaParam}` : ''}${refererParam ? `&referer=${encodeURIComponent(refererParam)}` : ''}`
      }).join('\n')
