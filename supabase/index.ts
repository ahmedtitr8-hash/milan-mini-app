// stream-proxy (نسخة Supabase Edge Functions / Deno) — نفس منطق نسخة Cloudflare Workers حرفيًا،
// بس بصيغة Deno. تُستخدم خصيصًا لروابط Xtream اللي تحظر نطاقات IP الخاصة بـ Cloudflare Workers.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    upstream = await fetch(targetUrl.toString(), {
      headers: {
        ...
        // بدون Referer عمدًا — تطبيقات IPTV الحقيقية ما ترسله، وإرساله كان يفضح الطلب كبروكسي/مشغل ويب
      },
      redirect: "follow",
    });
  } catch (e) {
  ...
  // مسار الفنكشن على Supabase يتضمن /functions/v1/stream-proxy — نحافظ عليه كاملاً بإعادة الكتابة
