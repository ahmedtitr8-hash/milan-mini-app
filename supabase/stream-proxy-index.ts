...
Deno.serve(async (req: Request) => {
  ...
  // DASH: نتحقق إذا الرد XML/MPD (سواء بدأ بـ <?xml أو مباشرة بـ <MPD) — بعض السيرفرات ما ترجع
  // إعلان XML وتبدأ مباشرة بالعنصر. هذا مستقل تمامًا عن اسم الملف (index.mpd / manifest.mpd / أي اسم)
  ...
  if (isMpd) {
    ...
    // نعيد كتابة كل الروابط الداخلية اللي ممكن يطلبها المشغل مباشرة بعد ما ياخذ المانفست:
    // BaseURL (سواء عنصر مستقل أو داخل Period/AdaptationSet/Representation)، وروابط
    // SegmentTemplate (initialization= / media=) اللي غالبًا فيها $Number$/$Time$ متغيرات —
    // نحافظ عليها كما هي داخل الرابط المُرمّز (encodeURIComponent ما يفكّها).
