...
Deno.serve(async (req: Request) => {
  ...
  // كشف مانفست DASH بنفس منطق كشف #EXTM3U فوق: من أول بايتات الرد الفعلية، بغض النظر عن
  // اسم الملف (index.mpd أو أي اسم ثاني) أو ترويسة Content-Type
  ...
  if (isDashManifest) {
    ...
    // نفس منطق تحويل روابط HLS الداخلية فوق، بس لعناصر/خواص DASH (BaseURL, initialization,
    // media, sourceURL) — قوالب $Number$/$Time$/... تُحفظ حرفيًا (نرجع %24 لـ$ بعد الترميز)
    // عشان المشغل يقدر يعوّض فيها، و& تتحول &amp; لأنها تنزرع داخل نص/خاصية XML
    const wrapXml = (raw: string) => {
      try {
        const abs = new URL(raw.trim(), targetUrl).toString();
        const encoded = encodeURIComponent(abs).replace(/%24/g, "$");
        const proxied = `${proxyBase}?url=${encoded}`;
