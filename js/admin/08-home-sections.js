...
function openSectionForm(section){
  ...
  document.getElementById('sf_banner_script').value = cfg.network_script || '';
...
async function saveSection(){
  ...
  if (type === 'clubs_grid'){
    ...
    const network_script = document.getElementById('sf_banner_script').value.trim();
    if (!image_url && !network_script){ toast('اكتب رابط صورة البانر أو كود إعلان', true); return; }
    config = { image_url, link_url, network_script };
  }
  if (editingSectionId){
