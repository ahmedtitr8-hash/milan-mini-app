...
function showPreRollAd(){
  return new Promise(resolve => {
    ...
    // يحترم القيمة اللي يحطها الأدمن بالضبط (كانت مقفولة بين 10-20 ثانية إجباريًا قبل كذا)
    const duration = Math.max(1, parseInt(adSettings.ad_duration_seconds, 10) || 15);
...
// كانت هذي الدالة معرّفة بدون أي استدعاء لها بالمشروع كامل، فتبقى adSettings=null للأبد
// ولا يظهر الإعلان مهما فعّلته من الأدمن. نناديها هنا فورًا (لا تحتاج غير sb، المعرّف
// مسبقًا بـ01-config.js) عشان تكون الإعدادات جاهزة قبل ما يفتح المستخدم أول مباراة.
loadAdSettings();
