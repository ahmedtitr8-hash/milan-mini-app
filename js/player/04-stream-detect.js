...
function isXtreamLiveUrl(url){
  ...
  // أي رابط ينتهي فعليًا بامتداد m3u8 (زي stream.m3u8 من أداتنا BarMi، أو ملفات CDN
  // حقيقية زي CloudFront) هو مانفست HLS مباشر، مو بث IPTV خام. بدون هالاستثناء، وجود
  // كلمة "live" بالمسار يخليه يُصنّف غلط كجهاز IPTV، فيمر ببروكسي يتنكر بمتصفح وهمي
  // (VLC) — ونفق Cloudflare يحجب هالتنكر ويكسر التشغيل. الحالات اللي فيها m3u8 مزيّف
  // يغلّف بث اكستريم حي (مقطع وحيد بلا نهاية) تنكشف بفحص المحتوى الفعلي عبر
  // tryUnwrapFakeXtreamPlaylist، مو تخمين من شكل الرابط.
...
// تجربة: مصادر m3u8 العادية (غير Xtream الخام) تمر الآن ببروكسي Cloudflare بدل Supabase.
// للرجوع: رجّع القيمة لـ `https://ckriyvqnrzravknajckl.supabase.co/functions/v1/stream-proxy`
const STREAM_PROXY = `https://milan-mini-app.ahmedtitr8.workers.dev`;
// روابط Xtream الخام (.ts) تبقى على Supabase — مؤكد إنها محظورة على Cloudflare، ما فيه داعي نعيد تجربتها
