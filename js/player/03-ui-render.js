...
function loadTabSources(){
  ...
  if (list.length){
    // البث يبدأ تحميله فورًا بالتوازي مع الإعلان (يشتغل بالخلفية جوه playerWrap)،
    // مو بعد ما يخلص الإعلان — كذا ما ينتظر المستخدم مرتين (إعلان ثم تحميل).
    // الإعلان يبقى فوقه بصريًا (z-index:30) لين ما يخلص/يُتخطّى.
    loadSource(list[0].url, list[0]);
    showPreRollAd();
    startSideAdCycle(list[0].created_at); // توقيت عالمي موحّد من وقت إضافة السيرفر — راجع 12-side-ad.js
...
function showUpcomingState(m){
  ...
  // نمسح "بصمة" حماية السبينر بـshowPlaceholder — بدونها، أول استدعاء تالٍ لـshowPlaceholder
  // بنفس رسالة "جارِ تحميل البث…" (اللي تصير عادةً فور اختيار مباراة ثانية) بيفتكر إن نفس
  // المحتوى معروض أصلاً ويرفض يعيد الرسم، فيفضل هالعداد المتجمد ظاهر بالشاشة بدون تحديث
  delete p.dataset.mode;
  delete p.dataset.msg;
