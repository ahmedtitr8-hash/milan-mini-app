...
async function loadSource(url, source){
  ...
  let triedDirectHttp = false;
  let playUrl = rawUrl;
  if ((/^http:\/\//i.test(rawUrl) || isXtreamSource) && primaryEngine !== 'native') {
    // عنصر <video> الأصلي ما يحتاج CORS للتشغيل العادي (بعكس hls.js/Shaka اللي تجيب
    // البيانات ببرمجة JS عبر fetch) — فملفات mp4/webm العادية تروح مباشرة على رابطها
    // الأصلي بدون بروكسي، بالضبط زي ما كان يشتغل بالتطبيق القديم البسيط. هالشي يفادي
    // قيد "مهلة الخمول" (idle timeout) اللي فرضها البروكسي وقت طلب المتصفح لآخر الملف
    // (moov atom بملفات ما انصدرت بـfaststart) — البروكسي يفضل احتياطي فقط لو فشل المباشر.
    playUrl = `${PROXY_BASE}?url=${encodeURIComponent(rawUrl)}${isXtreamSource ? '' : '&ua=browser'}`;
  } else if (primaryEngine === 'native' && /^http:\/\//i.test(rawUrl)) {
    // ملف mp4 عادي برابط http: نجرب أول شي نسخة https من نفس الرابط/الدومين مباشرة
    // (أغلب مزودي الاستضافة زي koyeb يدعمون https تلقائيًا على نفس النطاق) — هالشي يفادي
    // احتمال حظر "محتوى مختلط" (تحميل http داخل صفحة https) بدون ما نحتاج نمر بالبروكسي إطلاقًا
    playUrl = rawUrl.replace(/^http:\/\//i, 'https://');
    triedDirectHttp = true;
  }
  ...
  if (adminForcedLive){
  ...
  } else if (!isXtreamSource && primaryEngine !== 'mpegts' && primaryEngine !== 'native' && !adminForcedVod){
    // ملف فيديو عادي (mp4/webm/...) بدون تحديد نوع صريح من الأدمن: يستبعد من فرع شاكا هذا
    // ويروح مباشرة لفرع "else" تحت (اللي يستخدم عنصر <video> الأصلي عبر playNative). بدون
    // هالاستثناء، رابط mp4 عادي كان يُمرَّر لشاكا (مصممة لمانيفست HLS/DASH) عبر رابط بروكسي
    // بلا امتداد ظاهر بآخره، فتفشل بصمت أو تماطل — رغم إن البروكسي نفسه يرجّع الرد صحيح تمامًا
    // (Content-Type: video/mp4 مع دعم Range كامل)، لأن المشكلة كانت باختيار المحرك مو بالشبكة.
    const proxiedUrl = `${PROXY_BASE}?url=${encodeURIComponent(rawUrl)}`;
    const proxiedUrlMx = `${PROXY_BASE}?url=${encodeURIComponent(rawUrl)}&ua=mx`;
    ...
    if (result !== 'ok' && !isXtreamSource && (playUrl === rawUrl || triedDirectHttp)){
