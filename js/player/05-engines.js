...
function hidePlaceholder(){
  document.getElementById('playerPlaceholder').classList.add('hide');
  const v = video();
  if (v) v.classList.add('video-visible');
}
...
async function resetPlayerUI(){
  ...
  v.classList.remove('video-visible');
  ...
  // تصفير فوري لبقايا واجهة المصدر/المباراة السابقة (شريط التقدم، الوقت، العداد التنازلي)
  // بدون هذا، هالعناصر تفضل عارضة قيم القديم لين الفيديو الجديد يبدأ فعليًا ويطلق أحداثه
  // الخاصة — فيحس المستخدم إن الضغطة "ما استجابت" رغم إن التبديل صار فعليًا من أول لحظة
  try{ clearInterval(countdownTimer); }catch(e){}
  const seekBar = document.getElementById('seekBar');
  if (seekBar){ seekBar.value = 0; updateSeekFill(seekBar, 0); }
...
async function playShaka(url, drmKey, myGen){
  ...
  // تشخيص: بدون هذا، أي خطأ فك تشفير يصير أثناء التشغيل (بعد نجاح تحميل المانفست)
  // يمر بصمت تام — الفيديو يضل عالق بدون أي رسالة أو سجل يوضح السبب. هذا بالضبط
  // النمط اللي يفسّر "يجيب الجودات بس ما يبدأ التشغيل": المانفست تحمّل صح، لكن
  // طلب المفتاح الفعلي وقت فك تشفير أول مقطع فيديو يفشل بصمت.
  shakaInst.addEventListener('error', (event)=>{
    const detail = event.detail || {};
    console.error('[Shaka error]', 'code:', detail.code, 'category:', detail.category, detail);
    if (loadGeneration === myGen){
      showToastMsg(`⚠️ خطأ تشغيل/تشفير (${detail.code || '؟'}) — جرّب سيرفر ثاني`, 5000);
    }
  });
  ...
  // حماية إضافية: لو الفيديو ما تحرّك إطلاقًا خلال 6 ثواني من بدء التشغيل (علامة
  // نموذجية لتجمّد فك التشفير بصمت رغم نجاح تحميل المانفست وظهور الجودات)، ننبّه
  // بدل ما يضل المستخدم شايف شاشة عالقة بدون أي تفسير
  const stallCheckStart = v.currentTime;
  setTimeout(()=>{
    if (loadGeneration !== myGen) return;
    if (currentEngine === 'shaka' && !v.paused && v.currentTime <= stallCheckStart + 0.05){
      console.warn('[Shaka] تجمّد التشغيل بعد التحميل الناجح — غالبًا فشل صامت بفك التشفير');
      showToastMsg('⚠️ البث تجمّد بعد التحميل — جرّب سيرفر ثاني أو أعد المحاولة', 5000);
    }
  }, 6000);
  setupQualityMenu();
}
